function SlidingWindow_Analysis(files_in,files_out),
...
for pp = 1:length(pnames),
    ...
    for rr = 1:length(files_in.run),
        ...
        load(strcat( files_in.path, pnames{pp},'_',files_in.run{rr},'.mat' ));
        ...
        sNam = strcat( pnames{pp},'_',files_in.run{rr},'.mat' );
        save( strcat( files_out,sNam ) , 'CorrMat', 'labels_rois', 'mask', 'WinSiz');
    end
end
end
